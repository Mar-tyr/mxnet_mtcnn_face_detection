...
def track_video_face(video_dir, subject_id, outdir):
    detector = MtcnnDetector(model_folder='model', ctx=mx.cpu(0), num_worker=8, accurate_landmark=False)
    video_path = video_dir / 'P{}.mp4'.format(subject_id)
    clip = VideoFileClip(str(video_path))
    clip = resize(clip, (160, 90))
    face_frames = []
    first_undetected_n = 0
    for i, frame in tqdm(enumerate(clip.iter_frames()), desc=str(subject_id)):
        try:
            ...
            first_undetected_n += 1
    print('Need to fill to beginning:', first_undetected_n)
    for i in range(first_undetected_n):
        face_frames.insert(0, face_frames[0])
...
if __name__ == '__main__':
    ...
    subject_ids = config['ids']
    for subject_id in subject_ids:
        track_video_face(video_dir, subject_id, outdir)
