    ...
    for i, frame in tqdm(enumerate(clip.iter_frames())):
        ...
        if results is not None:
            ...
            for box in total_boxes:
                # draw = frame.copy()
                # cv2.rectangle(draw, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (255,255,255))
                if (box[2] - box[0]) > 24 and (box[3] - box[1]) > 24:
                    ...
                    if i == 0:
                        ...
                        sml = ssim(last_face_gray, tmp_face_gray, data_range=(tmp_face_gray.max() - tmp_face_gray.min()))
                        if sml > 0.3:
                            x1, y1, x2, y2 = tmp_x1, tmp_y1, tmp_x2, tmp_y2
                            face = tmp_face
                        else:
